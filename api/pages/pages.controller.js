...
module.exports.destroy = (req, res) => {
  ...
  return Pages.findById(id).exec()
    ...
    .catch(handleError(res))
    .then(() => tools.delFile(id))
    .then(() => tools.delGulpFolder(id))
    .catch(error => console.log(error))
}

module.exports.uploadById = (req, res) => {
  return tools.gulpFileSvn(req.params.id, req.user.loginId, req.user.project)
    .then(res => {
      json = JSON.parse(res)
      if (!json.status) {
        throw new Error(json.msg)
      }
      return Pages.findOneAndUpdate({ _id: req.params.id }, { uploadDate: new Date().getTime() }).exec()
    })
    .then(respondWithResult(res))
    .catch(handleError(res))
}
