    ...
    <div class="content">
      ...
      <div class="setting">
        ...
        <div class="qrcode">
          ...
          <div class="upload-link">发布后线上地址：<span class="copy-upload">
            {{project==='bilin'?'http://pgbilin.yy.com/statictpl/':'http://page.yy.com/friend_h5maker/'}} {{this.itemId.slice(0, 12)}}/index.html
          </span></div>
          <div class="release" @click="release"><el-button style="width:180px" type="primary" icon="upload">发 布</el-button></div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    release () {
      ...
      this.$store.dispatch('uploadTheme', tools.vue2json(this.$store.state.editor.editorTheme)).then(() => {
        this.$message({
          message: '发布成功',
          type: 'success'
        })
